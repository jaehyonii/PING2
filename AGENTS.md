...
- 하단 탭 네비게이션은 `(tabs)` 라우트 그룹에만 적용됨
- `/camera`는 그룹 밖이라 탭바가 표시되지 않음

## 라우팅 규칙
- `src/app/(tabs)/page.js` -> `/`
- `src/app/(tabs)/events/page.js` -> `/events`
- `src/app/(tabs)/leaderboard/page.js` -> `/leaderboard`
- `src/app/(tabs)/profile/page.js` -> `/profile`
- `src/app/camera/page.js` -> `/camera`

## 네비게이션 구조
- 레이아웃: `src/app/(tabs)/layout.js`
- 링크: 홈, 이벤트, 카메라, 리더보드, 프로필
- 카메라 화면에서는 하단 네비게이션 미표시

## 스타일 상태
- `src/app/globals.css`는 순수 CSS
- Tailwind는 `package.json`에 설치되어 있으나 아직 적용되지 않음

## 문의/진행 상태
- Figma 홈 디자인 구현 요청이 있었으나 접근 권한/공개 링크 필요
- 공유 링크 공개 또는 캡처/스펙 제공 시 구현 진행 가능
